...
// Json Parsing Code Start
            ...
            try {
                ...
                for (int i=0;i<jsonArray.length();i++)
                {
                    ...
                    double bk_gr = jsonObjectBahan.getDouble(DBHelper.BK_GR);
                    double tdn_gr = jsonObjectBahan.getDouble(DBHelper.TDN_GR);
                    double pk_gr = jsonObjectBahan.getDouble(DBHelper.PK_GR);
                    double ca = jsonObjectBahan.getDouble(DBHelper.CA);
                    double p = jsonObjectBahan.getDouble(DBHelper.P);
                    double bk_prs = jsonObjectBahan.getDouble(DBHelper.BK_PRS);
                    double tdn_prs = jsonObjectBahan.getDouble(DBHelper.TDN_PRS);
                    ...
                    bahan.setBk_gr(bk_gr);
                    bahan.setTdn_gr(tdn_gr);
                    bahan.setPk_gr(pk_gr);
                    bahan.setCa(ca);
                    bahan.setP(p);
                    bahan.setBk_prs(bk_prs);
                    bahan.setTdn_prs(tdn_prs);
