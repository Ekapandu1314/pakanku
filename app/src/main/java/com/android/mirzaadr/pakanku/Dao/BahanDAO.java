...
public class BahanDAO {
    ...
    public Bahan createBahan(String nama_bahan, double bk_gr, double tdn_gr,
                             double pk_gr, double ca, double p, double bk_prs,
                             double tdn_prs, double pk_prs, String kategori, int harga) {
        ContentValues values = new ContentValues();
        //values.put(DBHelper.BAHAN_ID, idbahan);
        values.put(DBHelper.NAMA_BAHAN, nama_bahan);
        values.put(DBHelper.BK_GR, bk_gr);
        values.put(DBHelper.TDN_GR, tdn_gr);
        values.put(DBHelper.PK_GR, pk_gr);
        values.put(DBHelper.CA, ca);
        values.put(DBHelper.P, p);
        values.put(DBHelper.BK_PRS, bk_prs);
        values.put(DBHelper.TDN_PRS, tdn_prs);
    ...
    public Bahan getBahanById(int id) {
        Cursor cursorxx = mDatabase.query(DBHelper.TABLE_BAHAN, mAllColumns, DBHelper.BAHAN_ID + " = ",
                new String[] { String.valueOf(id) }, null, null, null, null);
        if (cursorxx != null)
            cursorxx.moveToFirst();

        Bahan bahan = cursorToBahan(cursorxx);
        // return contact
        return bahan;
        ...
        try {
            ...
            if (!cursor.isLast()) {
                while (cursor.moveToNext()) {
                    ...
                    bahan.setBk_gr(cursor.getDouble(2));
                    bahan.setTdn_gr(cursor.getDouble(3));
                    bahan.setPk_gr(cursor.getDouble(4));
                    bahan.setCa(cursor.getDouble(5));
                    bahan.setP(cursor.getDouble(6));
                    bahan.setBk_prs(cursor.getDouble(7));
                    bahan.setTdn_prs(cursor.getDouble(8));
                    bahan.setPk_prs(cursor.getDouble(9));
                    bahan.setKategori(cursor.getString(10));
                    bahan.setHarga(cursor.getInt(11));
                    bahanList.add(bahan);
                }
            }
            //db.close();
